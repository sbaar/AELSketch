...
import org.sketchel.ArrangeMeasurement;
import org.sketchel.ArrangeMolecule;
import org.sketchel.Molecule;
import org.sketchel.RenderEffects;
import org.sketchel.SVGFont;
import org.sketchel.Util;
...
public class DrawMolecule
{
	...
	protected int backgr=Color.parseColor("white");
	protected int colHighlight=-1,colSelected=-1,colDragged=-1,colCurrent=-1;
	...
	public DrawMolecule(Molecule mol,Canvas gr,double angscale)
	{
		this.mol=mol;
		this.g=gr;
		this.scale=angscale;
		...
		measure=new ArrangeMeasurement()
		{
			...
			public double[] measureText(String str,double fontSize) {
					...
					return new double[]{fm.leading,fm.ascent, fm.descent};
			}
	...
	public int getBackground() {return backgr;}
	...
	public void setBackground(int col) {backgr=col;}
	...
	public void draw()
	{
		if (policy!=null)
		{
			...
			for (int n=1;n<=mol.numAtoms();n++)
			{
				...
				acol[n-1]=Color.BLACK; //anum>=0 && anum<policy.atomCols.length ? (policy.atomCols[anum].getRGB()&0xFFFFFF) : 0x000000;
			}
			arrmol.setForeground(policy.foreground &0xFFFFFF);//TODO: need and?
	...
	{
		if (colHighlight==-1) colHighlight=Color.CYAN;//backgr.darker();
		if (colSelected==-1) colSelected=Color.MAGENTA;//new Color(colHighlight.getRed(),colHighlight.getGreen(),255);
		if (colDragged==-1) colDragged=Color.BLUE;//new Color(colHighlight.getRed(),192,255);
		if (colCurrent==-1) colCurrent=Color.YELLOW;//Color.rgb(0,128,64);
	}
	...
	*/
	...
	private void drawBonds()
	{
		...
		for (int n=0;n<arrmol.numLines();n++)
		{
			...
			double x1=arrmol.lineX1(n),y1=arrmol.lineY1(n),x2=arrmol.lineX2(n),y2=arrmol.lineY2(n);
			drawOneBond(btype,x1,y1,x2,y2,(arrmol.lineCol(n)),arrmol.lineSize(n));
		}
		...
		if (effects.bondInProgress)
		{
			...
			else
			{
				...
				for (int i=0;i<effects.bipOrder;i++,v++) g.drawLine( (float) (x1+ox*v),(float) (y1+oy*v),(float) (x2+ox*v),(float) (y2+oy*v), p);
			}
		}
	}

	// workhorse function for above
	private void drawOneBond(int btype,double x1,double y1,double x2,double y2,int col,double sz)
	{
		x1 +=100;
		x2+=100;
		y1+=100;
		...
		{
			//g.setStroke(new BasicStroke((float)sz,BasicStroke.CAP_ROUND,BasicStroke.JOIN_ROUND)); TODO: round off
		...
		else if (btype==ArrangeMolecule.BLINE_DECLINED)
		{
			...
			for (int i=0;i<=nsteps+1;i++)
			{
				...
				if (Util.norm2(2*ix,2*iy)<=1)
				{
					...
					g.drawOval(myOval((float)(cx-ix), (float)(cy -iy), (float)(2*ix), (float)(2*iy)), p);
					continue;
				}
				p.setStrokeWidth(3);
			...
			//	g.draw(new Line2D.Double(cx-ix,cy-iy,cx+ix,cy+iy));
				g.drawLine((float) (cx-ix), (float)(cy-iy), (float) (cx+ix), (float) (cy+iy), p);
		...
		else if (btype==ArrangeMolecule.BLINE_UNKNOWN)
		{
			...
			for (int i=0;i<=nsteps;i++)
			{
				...
				Path path = new Path();
				//TODO quad curve
				//path.quadTo((float)ax, (float)ay, (float)(bx+sign*ox), (float)(by+sign*oy)));
		...
		else if (btype==ArrangeMolecule.BLINE_DOTTED || btype==ArrangeMolecule.BLINE_DOTDIR)
		{
			...
			for (int i=0;i<=nsteps+1;i++)
			{
				...
				g.drawRect(myOval((float)(cx-r), (float)(cy-r), (float)(2*r), (float)(2*r)), p);
		...
		else if (btype==ArrangeMolecule.BLINE_INCDOUBLE || btype==ArrangeMolecule.BLINE_INCTRIPLE)
		{
			...
			else
			{
				...
				//g.draw(new Line2D.Double(x1,y1,x2-0.33*ox,y2-0.33*oy));
				g.drawLine((float)x1, (float)y1, (float)(x2+0.33*ox), (float)(y2+0.33*oy), p);
				g.drawLine((float)x1, (float)y1, (float)(x2-0.33*ox), (float)(y2+0.33*oy), p);
			}
		}
	}
	...
	private void drawAtoms()
	{
		for (int n=0;n<arrmol.numPoints();n++)
		{
			...
			p.setColor(arrmol.pointCol(n));
			
			if (txt.equals("."))
			{
				double r=scale*0.1;
				//g.fill(new Ellipse2D.Double(arrmol.pointCX(n)-r,arrmol.pointCY(n)-r,2*r,2*r));
				g.drawRect(myOval((float) (arrmol.pointCX(n)-r),(float) (arrmol.pointCY(n)-r),(float) (2*r),(float) (2*r)), p);
			}
			else if (txt.equals("+"))
			{
				double x=arrmol.pointCX(n),y=arrmol.pointCY(n),r=scale*0.15;
				//TODO figure out strokes
				//g.setStroke(new BasicStroke((float)(0.07*scale),BasicStroke.CAP_ROUND,BasicStroke.JOIN_ROUND));
				g.drawLine((float)(x-r),(float) y,(float) (x+r), (float)y, p);
				g.drawLine((float)(x),(float) (y-r),(float) (x), (float)(y+r), p);
			...
			else // ordinary text
			{
				...
				float cx=(float)(arrmol.pointCX(n)),cy=(float)(arrmol.pointCY(n));
				...
				float x = arrmol.pointCX(n);
				float y = arrmol.pointCY(n);
				float offset = 20;
				g.drawText(txt, (arrmol.pointCX(n))+ offset, (arrmol.pointCY(n))+ offset, p);
				...
				if (anum!=0 && effects.underlined!=null && effects.underlined[anum-1])
				{
					...
					g.drawLine((float)(cx),(float) cy+2,(float) (cx+sw), (float)cy+2, p);
				}
			}
		}
	}
